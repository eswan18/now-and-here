...
from now_and_here.views.task_views import today
...
def test_today_simple(temp_store: DataStore):
    ...
    tasks = today(temp_store, UserContext(utc))
    assert len(tasks) == 3
    ...
    tasks = today(temp_store, UserContext(ZoneInfo("America/Chicago")))
    assert len(tasks) == 3
    ...
    tasks = today(temp_store, UserContext(ZoneInfo("Europe/Berlin")))
    assert len(tasks) == 2
